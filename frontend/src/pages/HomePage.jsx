...
import { CheckCheckIcon, MapPinIcon, UserIcon, UserPenIcon, UserPlusIcon } from "lucide-react";
import FriendCard from "../components/FriendCard.jsx";
import NoFriendsFound from "../components/NoFriendsFound.jsx";

const HomePage = () => {
  const queryClient = useQueryClient();
  const [ongoingRequestsIds, setOngoingRequestsIds] = useState(new Set());
  ...
  const { data:recommendedUsers=[], isLoading: loadingRecommendedUsere } =
    useQuery({
      queryKey: ["users"],
      queryFn: getRecommendedUsere,
    });
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="opacity-70">
                  Discover prefect partner to share your knowledge
                </p>
              </div>
            </div>
          </div>

          {loadingRecommendedUsere ? (
            <div className="flex justify-center py-12">
              <span className="loading loading-spinner loading-lg" />
            </div>
            ...
            <div className="card bg-base-200 p-6 text-center">
              ...
              <p className="text-base-content opacity-70">
                Check back later for new recommendation's
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              {recommendedUsers.map((user) => {
                const hasRequestBeenSent = ongoingRequestsIds.has(user._id);
